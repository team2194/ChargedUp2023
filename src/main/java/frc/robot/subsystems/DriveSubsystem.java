...
public class DriveSubsystem extends SubsystemBase {
  ...
  public void setAddVisionMeasurement(boolean add) {
    addVisionMeasurement = add;
  }
    ...
    if (checkCANOK()) {
      ...
      if (addVisionMeasurement) {
        ...
        m_poseEstimator.addVisionMeasurement(
            ...
            getVisionPoseEstimatedData(),

            Timer.getFPGATimestamp() - getPipelineLatencyMs());
      }
    }
  }

  public Pose2d getVisionPoseEstimatedData() {
    return getRobotPose_FS_WPIBL(LimelightHelpers.getBotpose_wpiRed(""));
  }

  public double getPipelineLatencyMs() {
    return LimelightHelpers.getLatency_Pipeline("");
  }

  public Pose2d getRobotPose_FS_WPIBL(double[] result) {
    Translation2d tl2d = new Translation2d(result[0], result[1]);
    Rotation2d r2d = new Rotation2d(Units.degreesToRadians(result[2]), Units.degreesToRadians(result[3]));
    Transform2d tf2d = new Transform2d(tl2d, r2d);
    Pose2d p2d = new Pose2d().transformBy(tf2d);
    return p2d;
  }
